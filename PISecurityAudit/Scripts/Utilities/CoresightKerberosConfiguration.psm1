...
$title = "PI DOG - Run this locally on the Coresight server."
...
$resultRSAT = $host.ui.PromptForChoice($titleRSAT, $messageRSAT, $optionsRSAT, 0) 
    ...
    If ($resultRSAT -eq 0) {
    Write-Host "Installation of 'Remote Active Directory Administration' module is about to start.."
    Add-WindowsFeature RSAT-AD-PowerShell
    }
    Else { Write-Host "'Remote Active Directory Administration' is required to check Kerberos Delegation settings. Aborting." 
    break
    }
...
#Generate root path used to grab web configuration properties
...
$CSUserAdm = Get-PISysAudit_IISproperties -lc $true -rcn $RemoteComputerName -qry $QueryAdmUser -DBGLevel $DBGLevel
    ...
    # Check whether a custom account is used to run the AppPool.
    ...
    $CSWebServerFQDN = $CSWebServerName + "." + $CSWebServerDomain 
        ...
        # By default, assume custom header is not assumed.
        ...
        $matches = [regex]::Matches($BindingsToString, ':{1}\d+:{1}(\S+)\s') 
            foreach ($match in $matches) { 
                $CSheader = $match.Groups[1].Captures[0].Value 
			        If ($CSheader) { # A custom host header is used! The first result is taken.
       ...
       # Check whether the custom host header is a CNAME Alias or Host (A) entry.
       ...
       $CNAME = $false 
            ...
            If ($blnCustomAccount) {
                ...
                # SPN check
                $svcAcc = $CSUserSvcSPN
                $spnCheck = $(setspn -l $svcAcc).ToLower()
                $spnCounter = 0
        ...
        If ($rbkcd) {
            ...
            $AFServers = [regex]::Matches($AFSDK, 'host=\"([^\"]*)') 
                    ...
                    foreach ($AFServerTemp in $AFServers) { 
                        ...
                        $test = Get-ADObject -Filter { Name -like $value } -Properties ObjectCategory | Select -ExpandProperty objectclass
                        If ($test -eq "user") { $AccType = 1 } ElseIf ($test -eq "computer") { $AccType = 2 } ElseIf ($test -eq "msDS-GroupManagedServiceAccount") {  $AccType = 3 } Else { "Unable to locate ADObject $test." }
            ...
            $PIServers = Get-ChildItem $regpath | ForEach-Object {Get-ItemProperty $_.pspath} | where-object {$_.path} | Foreach-Object {$_.path}
                    foreach ($PIServer in $PIServers) { 
                        ...
                        $test = Get-ADObject -Filter { Name -like $value } -Properties ObjectCategory | Select -ExpandProperty objectclass
                        If ($test -eq "user") { $AccType = 1 } ElseIf ($test -eq "computer") { $AccType = 2 } 
                        ElseIf ($test -eq "msDS-GroupManagedServiceAccount") {  $AccType = 3 } Else { "I DUNNO" }
        ...
        If ($CSAppPoolSvc -eq "NetworkService") { $CSUserSvc = $CSWebServerName  }
            If ($blnCustomAccount) { 
                ...
                $CSUserSvc = $CSUserSvc.Substring($posAppPool+1)
                    ...
                    Else { "Unable to locate ADObject $test." 
                    break
                    }
                ...
                If (!$blngMSA) {                
                    ...
                    Else {
                    Write-Output "CS AppPool identity is: $CSAppPoolSvc"
                    }
                }
                ...
                If ($blnDelegationCheckConfirmed) {
                    ...
                    If ($rbkcd) {
                    Write-Output "Coresight Application Pool: $global:RBKCDstring"
                    }
