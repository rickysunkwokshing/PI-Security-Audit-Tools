...
{				
	...
	try
	{										
		$ServiceAccount = Get-PISysAudit_ServiceProperty -sn 'afservice' -sp LogOnAccount -lc $LocalComputer -rcn $RemoteComputerName -dbgl $DBGLevel
...
{		
	...
	try
	{
		if($global:ArePowerShellToolsAvailable)
		{
			$version = [int]($global:AFServerConnection.ServerVersion -replace '\.', '')
